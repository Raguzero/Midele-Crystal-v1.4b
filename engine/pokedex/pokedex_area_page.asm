...
; GRASS_WILDDATA_LENGTH EQU 2 + 3 + NUM_GRASSMON * 2 * 3 ; vanilla is $2f, 47 bytes. 3 different times of day
; WATER_WILDDATA_LENGTH EQU 2 + 1 + NUM_WATERMON * 2 ; only 1 time of day
; FISHGROUP_DATA_LENGTH EQU 1 + 2 * 3

AREA_ENTRY_SIZE_BYTES EQU 2 ; vanilla is 2 bytes, 1 byte for lvl, 1 for species index. pk 16, species is 2 bytes
AREA_ENTRY_SKIP_ENCOUNTER_RATES EQU 6 ; see the wild data: db 2 percent, 2 percent, 2 percent ; encounter rates: morn/day/nite
AREA_ENTRY_TIMEOFDAY_PRINT_SPACING EQU 6

AREA_MORN_ICON EQU $65 ; from gfx\pokedex\pokedex.2bpp
AREA_DAY_ICON EQU $6b ; from gfx\pokedex\pokedex.2bpp
AREA_NITE_ICON EQU $6c ; from gfx\pokedex\pokedex.2bpp

MAX_ENTRIES_PER_PAGE EQU 3 ; we have 7 lines per page, and each entry takes two lines
MAX_LINES_PRINTED_PER_PAGE EQU MAX_ENTRIES_PER_PAGE * 2 ; 6
...
.print_map_id
	...
	ld a, BANK(CasinoMons)
	call GetFarHalfword ; hl contains coins
...
.loop
	ld a, 7 ;  NUM_NPC_TRADES DA ERROR AL COMPILAR, EL NUMERO 7 ES CORRECTO (NUMERO DE TRADES)?
	cp b ; count, trade entry index
	jr z, .donedone
...
.found
	...
	ld a, BANK(EventWildMons)
	call GetFarHalfword ; event flag constant in 'hl'
...
; TOGGLE EVENT CHECK:
	...
	ld a, BANK(EventWildMons)
	call GetFarHalfword ; map_id in 'hl'	
	ld d, h 
	ld e, l
	...
	ld a, BANK(EventWildMons)
	call GetFarHalfword ; blurb hint ptr in 'hl'
